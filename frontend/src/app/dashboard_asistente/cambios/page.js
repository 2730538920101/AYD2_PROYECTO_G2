...
function ListaConductores() {
    ...
    const obtenerConductores = async () => {
        try {
            const response = await handleAxios().get('conductores/solicitudes-cambio');
    ...
    const handleAprobar = async (con_id) => {
        try {
            await handleAxios().put(`conductores/aprobar-solicitud/${con_id}`);
            MySwal.fire({
                title: "Aprobación",
                text: "Solicitud de cambio aprobada",
                icon: "success",
            });
            obtenerConductores();
        } catch (error) {
    ...
    // Función para denegar un conductor
    const handleDenegar = async (con_id) => {
        try {
            await handleAxios().put(`conductores/denegar-solicitud/${con_id}`);
            MySwal.fire({
                title: "Denegación",
                text: "Solicitud de cambio denegada",
                icon: "warning",
            });
            obtenerConductores();
        } catch (error) {
    ...
    const columnas = [
        ...
        },
        {
            ...
            cell: row => (
                <>
                    <Button variant="success" onClick={() => handleAprobar(row.sol_id)}>
                        <FontAwesomeIcon icon={faPlaneArrival} /> Aprobar
                    </Button>
                    &nbsp;
                    <Button variant="danger" onClick={() => handleDenegar(row.sol_id)}>
                        <FontAwesomeIcon icon={faDeleteLeft} /> Denegar
                    </Button>
                </>
            )
        },
